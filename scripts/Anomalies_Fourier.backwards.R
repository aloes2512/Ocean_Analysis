df=readRDS("data/Ocean_anomaly.phaselocked.rds") #2068 obs
library(tidyverse)
# median not used here
Ocean_anoma<-df$Ocean_anomaly_detrend
N=NROW(Ocean_anoma) #2068
# eliminate annual and semianual (at equator)
library(itsmr)
M=c("season",12,"season",6)
Ocean_anomaly=Ocean_anoma%>%  mutate(anom=Resid(Ocean_Anomaly,M),
                                     trd3=trend(anom,3),
                                     res3=Resid(anom,3))
# format date as numeric (year decimals)
library(lubridate)
Ocean_anomaly=Ocean_anomaly%>%rename("dt.mnth"=Time)
N=NROW(Ocean_anomaly)
...
Ocean_solar_anom=Ocean_anomaly%>%
  ...
  dplyr::select(dt.mnth,anom,"Solar_Variation"=SI,trd3)
...
FFT.har.trd=tibble(idx=1:N-1,
                   ...
                   amp=Mod(spc))
FFT.har.trd%>%arrange(desc(amp))%>%subset(idx<N/2)
library(itsmr)
...
Long_periods=Ocean_solar_anom%>%
  mutate(smth.res=smooth.fft(Clean_Baseline_Trend ,f=0.02))
