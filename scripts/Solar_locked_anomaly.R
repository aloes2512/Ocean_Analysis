...
Ocean_solar_anom%>%ggplot(aes(x=dt.mnth))+
  geom_line(aes(y=trend.solar,col="sol.trnd"))+
  geom_line(aes(y=trd5,col="poly5.trnd"))+
  geom_line(aes(y=difference,col="differnc"))
saveRDS(Ocean_solar_anom,"data/Ocean_solar_anom.rds")
#______alternative method-------
library(mgcv)
gam_model <- gam(Anomaly ~ s(dt.mnth, k = 180, bs = "cc"), data = Ocean_solar_anom)
Ocean_solar_anom$gamtrd180<-predict(gam_model)
Ocean_solar_anom%>%ggplot(aes(x=dt.mnth))+
  geom_line(aes(y=gamtrd180,col="gam180trd"))+
  geom_line(aes(y=trend.solar,col="Baseline_Trend"))+
  labs(x="",title = "Trend-Comparison: GAM spline vs\n Solar-Power Phase locked")
ggsave("figs/trend_gam.solar.png")
#--------
...
Ocean_solar_anom%>%ggplot(aes(x=dt.mnth))+
  ...
  labs(title="Clean_Baseline_Trend",
       subtitle = "extracted with spline fit solar power 3rd.°")
#===========
...
library(itsmr)
Ocean_solar_anom=Ocean_solar_anom%>%
  mutate(max.hr.trd=hr(har.trd,N/idx.mx)) # periods manually from FFT.har.trd
#========
...
Long_periods%>%ggplot(aes(x=dt.mnth,y=trend.solar))+
  ...
  geom_line(aes(y=trend.solar-trd3),col=9)+
